...
fMCEvent(0),fDigMan(0),fGeo(0),fGeo_TOF(0),AS_Event(0),AS_Track(0),AS_Tracklet(0),AS_offline_Tracklet(0),AS_Digit(0),Tree_AS_Event(0),
TRD_ST_Tracklet(0),TRD_ST_TOF_hit(0),TRD_ST_MC_particle(0),TRD_ST_TPC_Track(0),TRD_ST_Event(0),Tree_TRD_ST_Event(0), h_ADC(0x0), fEventNoInFile(-2), N_good_events(0), N_total_events(0), fDigitsLoadedFlag(kFALSE),N_time_bins(24),
fListOfHistos(0x0),fTree(0x0), fPIDResponse(0), EsdTrackCuts(0),aliHelix(),aliHelix_TRD(),TV3_SVD_tracklet_offset(),TV3_SVD_tracklet_dir(),
...
void Ali_make_tracklets_from_digits::UserExec(Option_t *)
{
		    ...
		    Int_t fBaseline = fDigMan->GetDigitsParam()->GetADCbaseline(i_det); // ADC baseline to be subtracted from digit ADC values, constant value of 10
                    ...
                    if(ADC_amplitude > 0.0) // change to 0
                    {
                        ...
                        //    Float_t GainFactor = KryptoGain ->GetGainCorrectionFactor(i_det,i_row,i_column);
                            ...
                            //printf("N_total_events: %d, i_det: %d, ADC_amplitude: %4.3f, col: %d, row: %d, time: %d, GainFactor: %4.3f, chamber gain: %4.3f \n",N_total_events,i_det,ADC_amplitude,i_column,i_row,i_time,GainFactor,valuegainiteration); // XALEX comment after test
                        //}
	...
	//Helix
        ...
        AS_Track ->setHelix_TRD(aliHelix_TRD.fHelix[0],aliHelix_TRD.fHelix[1],aliHelix_TRD.fHelix[2],aliHelix_TRD.fHelix[3],aliHelix_TRD.fHelix[4],aliHelix_TRD.fHelix[5],aliHelix_TRD.fHelix[6],aliHelix_TRD.fHelix[7],aliHelix_TRD.fHelix[8]);
    ...
    for(UShort_t i_track = 0; i_track < NumTracks; ++i_track) // loop over all tracks of the actual event
    {
        ...
        TRD_ST_TPC_Track ->setHelix_TRD(helix_TRD_par[0],helix_TRD_par[1],helix_TRD_par[2],helix_TRD_par[3],helix_TRD_par[4],helix_TRD_par[5]);
...
void Ali_make_tracklets_from_digits::FillHelix(AliESDtrack* track_in, Double_t magF_in)
{
    ...
    for(Int_t i_ext_params = 0; i_ext_params < 2;  i_ext_params++) // loop over different helix parameters
    {
        ...
        //Float_t sphi  = TMath::Abs(fHelix_alt_par2_sav) < kAlmost1 ? asinf(fHelix_alt_par2_sav) : TMath::Sign(TMath::Pi()/2, fHelix_alt_par2_sav);
        //fHelix_alt_par2_sav = sphi + alpha_alt; // phi0 // RS : use float version
        //printf("par2: {%4.3f, %4.3f} \n",fHelix_alt[2],fHelix_alt_par2_sav);
        //-----------------
